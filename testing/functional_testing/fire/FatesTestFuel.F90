program FatesTestFuel
  ...
  class(fire_weather),               pointer     :: fireWeather_NI        ! fire weather object for nesterov index
  class(fire_weather),               pointer     :: fireWeather_CFW       ! fire weather object for canadian fire weather system
  ...
  real(r8),                          allocatable :: FFMC(:)               ! fine fuel moisture code
  real(r8),                          allocatable :: DMC(:)                ! duff moisture code
  ...
  real(r8),                          allocatable :: fuel_moisture_NI(:,:) ! fuel moisture calculated using NI [m3/m3]
  real(r8),                          allocatable :: fuel_moisture_CF(:,:) ! fuel moisture calculated using canadian fire weather [m3/m3]
  ...
  real(r8),               parameter :: latitude = 65.0_r8       ! latitude to run [degrees]
  integer, dimension(12), parameter :: last_day = [31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365] ! last julian day in month
  ...
  allocate(NI(n_days))
  allocate(FFMC(n_days))
  allocate(DMC(n_days))
  allocate(fuel_moisture_NI(n_days, num_fuel_models))
  allocate(fuel_moisture_CF(n_days, num_fuel_models))
  ...
  ! set up fire weather classes
  allocate(nesterov_index :: fireWeather_NI)
  allocate(canadian_fire_weather :: fireWeather_CFW)
  call fireWeather_CFW%Init()
  call fireWeather_NI%Init()
  ...
  do i = 1, n_days
    ...
    ! increment hlm_current_month
    if (i > last_day(hlm_current_month)) hlm_current_month = hlm_current_month + 1
    
    call fireWeather_NI%UpdateIndex(temp_degC(i), precip(i), rh(i), wind(i), latitude)
    call fireWeather_CFW%UpdateIndex(temp_degC(i), precip(i), rh(i), wind(i), latitude)
    
    NI(i) = fireWeather_NI%fire_weather_index
    
    select type (fireWeather_CFW)
      class is (canadian_fire_weather)
        FFMC(i) = fireWeather_CFW%ffmc
        DMC(i) = fireWeather_CFW%dmc
    end select
    
    ! calculate fuel moisture [m3/m3]
    do f = 1, num_fuel_models
      call fuel(f)%UpdateFuelMoisture(SF_val_SAV, SF_val_drying_ratio, fireWeather_NI)
      fuel_moisture_NI(i, f) = fuel(f)%average_moisture
      
      call fuel(f)%UpdateFuelMoisture(SF_val_SAV, SF_val_drying_ratio, fireWeather_CFW)
      fuel_moisture_CF(i, f) = fuel(f)%average_moisture
    end do
  end do 
  
  ! write out data
  call WriteFireData(out_file, n_days, num_fuel_models, temp_degC, precip, rh, NI,       &
    FFMC, DMC, fuel_loading, frac_loading, fuel_BD, fuel_SAV, total_loading,             &
    fuel_moisture_NI, fuel_moisture_CF, fuel_models, carriers)
